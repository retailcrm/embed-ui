...
describe('order/card', () => {
  test('store serves bidirectional connection', async () => {
    ...
    const host = createEndpoint<EndpointApi<HostMethods, EventSchema>>(fromMessagePort(port1))

    createWidgetEndpoint<EndpointApi<RemoteMethods, EventSchema>>({
    ...
    const wrapper = mount({
      setup () {
        ...
        const field = <F extends keyof FieldSchema>(name: F) => computed({
          get: () => store[name],
          set: (value: FieldSchema[F]) => store.set(name, value),
        })

        const email = field('customer.email')
        const phone = field('customer.phone')
        const address = field('delivery.address')
