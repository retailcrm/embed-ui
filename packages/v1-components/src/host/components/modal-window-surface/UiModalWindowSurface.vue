...
const props = defineProps({
  ...
  id: {
    type: String,
    default: () => uid('embed-modal-window-surface'),
  },
...
const scrollable = ref<{ updateScroll: InstanceType<typeof UiScrollBox>['update'] } | null>(null)
...
defineExpose({
  open: show,
  close,
  updateScroll: () => scrollable.value?.updateScroll(),
})
...
const EmbedModalWindowSurface = () => !state.attached ? undefined : h(Teleport, {
  ...
  default: () => withDirectives(h('div', {
    ...
    onScroll: (event: Event) => {
      if (!state.overlapped) {
        emit('scroll:y', event, body.value?.getBoundingClientRect() ?? null)
      }
    },
  }, {
    default: renderBody,
...
<style lang="less" src="../modal-window/modal-window.less" />
