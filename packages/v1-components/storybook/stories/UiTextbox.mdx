...
* `autofit` – Автоподгонка ширины `input/textarea` по мере ввода. Тип: `boolean`. По умолчанию: `false`. Примечание: итоговая ширина ограничена `max-width` корневого элемента компонента.
